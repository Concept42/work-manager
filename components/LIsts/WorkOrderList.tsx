  ...
  return (
    <>
      {workOrders ? (
        <tr>
          <td>
            <Link href={`/workorders/${workOrders.id}`}>
          ...
          {workOrders.customer ? <td>{workOrders.customer?.companyName}</td> : ''}

          <td>{workOrders.title}</td>
          <td>{workOrders.discription.slice(0, 10)}</td>
          {workOrders.user ? <td>{workOrders.user?.name}</td> : ''}
          <td>
